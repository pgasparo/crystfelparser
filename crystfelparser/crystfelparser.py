            ...
            if "Begin chunk" in line:
                ...
                if tmpframe["indexed_by"] != "none":
                    ...
                    tmpframe["Cell parameters"] = np.hstack(
                        [line[2:5], line[6:9]]).astype(np.float)

                    # Get the reciprocal unit cell as a 3x3 matrix
                    reciprocal_cell = []
                    for tmpc in range(3):
                        reciprocal_cell.append(
                            text_file.readline().split()[2:5])
                        ln += 1
                    tmpframe["reciprocal_cell_matrix"] = np.asarray(
                        reciprocal_cell).astype(np.float)
                    ...
                    ln += 6
                    tmpframe["diffraction_resolution_limit [nm^-1]"] = np.float(
                        line[2])
                    tmpframe["diffraction_resolution_limit [A]"] = np.float(
                        line[5])
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Console script to parse indexamajig.')
